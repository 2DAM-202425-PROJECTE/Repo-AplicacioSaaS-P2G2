    ...
    <Modal class="modal-dialog modal-lg">
            ...
            <form @submit.prevent="updateReserva">
                <div class="mb-4">
                    ...
                    <input type="text" v-model="form.telefon" id="telefon" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
                </div>
                <div class="mb-4">
                    <label for="data" class="block text-sm font-medium text-gray-700">Data:</label>
                    <input type="date" v-model="form.data" id="data" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
                </div>
                <div class="mb-4">
                    <label for="hora" class="block text-sm font-medium text-gray-700">Hora:</label>
                    <input type="time" v-model="form.hora" id="hora" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
                </div>
                <div class="mb-4">
                    <label for="num_persones" class="block text-sm font-medium text-gray-700">Persones:</label>
                    <input type="number" v-model="form.num_persones" id="num_persones" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
                </div>
                <div class="mb-4">
                ...
                <div class="flex justify-end">
                    <button type="button" @click="cancelEdit" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded mr-2">Cancel·lar</button>
...
const updateReserva = () => {
    router.put(route('reserves.update', { id: form.id }), form, {
        onSuccess: () => {
            router.visit(route('reserves.index', { restaurant: form.id_restaurant }));
        },
        onError: (errors) => {
